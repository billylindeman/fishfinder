    ...
    fn transform(scope: &thread::Scope<'env>, mut src: Consumer<u8>) -> Consumer<ModeSFrame> {
        ...
        scope.spawn(move |_| {
            ...
            loop {
                ...
                debug!(
                    "found ads-b frame {:?}",
                    hex::encode(&frame_bytes[0..msglen])
                );
                ...
                let crc2: u32 = sdr::crc::modes_checksum(&frame_bytes);
                let valid = crc == crc2;
                ...
                if valid {
                    frame_producer
                        .push(frame_bytes[0..msglen].into())
                        .expect("error pushing mode-s frame");
                } else if msgtype == 17 || msgtype == 11 {
                    debug!("crc mismatch, attempting bit repair on frame");
                    if let Some(repaired_frame) =
                        sdr::crc::modes_repair_single_bit(&frame_bytes[0..msglen].into())
                    {
                        info!(
                            "repaired frame {} => {}",
                            hex::encode(&frame_bytes[0..msglen]),
